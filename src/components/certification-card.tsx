...
export function CertificationCard({ certification }: { certification: Certification }) {
  return (
    ...
    >
      <a href={certification.link} target="_blank" rel="noopener noreferrer" className="block h-full p-4">
        <div className="flex flex-col h-full">
          <div className="flex items-center space-x-4 mb-4">
            ...
            />
            <div>
              <h3 className="text-sm font-medium leading-none mb-1">{certification.name}</h3>
              <p className="text-xs text-muted-foreground">{certification.company}</p>
            </div>
          </div>
          <div className="mt-auto flex justify-between items-center text-xs text-muted-foreground">
            <p>Received: {new Date(certification.receivedDate).toLocaleDateString()}</p>
